...
def find_scene_dirs(root: Path, requested_scene_ids: Iterable[str] | None) -> list[Path]:
    ...
    for sid in requested_scene_ids:
        scene_dir = root / sid
        scene_dir_suffixed = root / (sid + "_individual_objs")
        if scene_dir.exists():
            existing.append(scene_dir)
        elif scene_dir_suffixed.exists():
            existing.append(scene_dir_suffixed)
        else:
...
def load_scene_meshes(
    ...
    for path in paths:
        ...
        if transform_source == "json":
            ...
            # Skip OBJ files whose index exceeds the JSON object count.
            # DiffuScene may export more meshes than recorded in the JSON
            # (e.g. objects generated before end-token filtering).
            n_objects_in_json = len(scene_payload["translations"])
            if object_index >= n_objects_in_json:
                if debug_index:
                    print("[index]", path.name)
                    print("  filename_object_index =", object_index)
                    print("  skipped: index >= json object count", n_objects_in_json)
                continue
...
def main() -> None:
    ...
    for scene_idx, scene_dir in enumerate(scene_dirs, start=1):
        # Strip DiffuScene's "_individual_objs" suffix to match JSON scene_ids
        scene_key = scene_dir.name.removesuffix("_individual_objs")
